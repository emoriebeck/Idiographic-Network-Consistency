...
setwd("~/Dropbox/network/Bringmann 2016 R code/dblcentered")
getwd()
...
#%###########################################################%#
formula_data<-paste(getwd(),"/esm_w1_dblcentered.csv",sep="")
...
pvalueslist=list()
for (i in 1:nv){
  formula1=paste(getwd(),"/modelfixeddblcentered",i,".txt",sep="")
  fixed <-as.vector(unlist(read.table(formula1, quote="\"")))
  
  formula2=paste(getwd(),"/modelpvaluesdblcentered",i,".txt",sep="")
...
Network<-qgraph(E, layout="spring", curve = -1, cut = 0.05, esize = 10, 
...
edges=list()
for (i in 1:nv){
  formula=paste(getwd(),"/modelrandomdblcentered",i,".txt",sep="")
  random <- read.csv(formula, header=FALSE)
  #save the edges as a list
  edges[[i]]<-random[,-1]}
dataPAIRSedges=matrix(unlist(edges),ncol=(nv*nv)) #The columns are the links in the network (36) and the rows are the individuals (95).
...
for(i in 1:4){
  ...
  for (j in 1:nv){
    ...
    CentralityPAIRS[j,odd_indexes[i]]<-cor(output[j,],DataPAIRS[1:225,15])
    CentralityPAIRS[j,even_indexes[i]]<-round(cor.test(output[j,],DataPAIRS[1:225,15])$p.value,4)
  }
}
