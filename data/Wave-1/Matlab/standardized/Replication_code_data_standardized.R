...
setwd("~/Dropbox/network/Bringmann 2016 R code/standardized")
getwd()
...
#%###########################################################%#
formula_data<-paste(getwd(),"/esm_w1_standardized.csv",sep="")
...
pvalueslist=list()
for (i in 1:nv){
  formula1=paste(getwd(),"/modelfixedstandarized",i,".txt",sep="")
  fixed <-as.vector(unlist(read.table(formula1, quote="\"")))
  
  formula2=paste(getwd(),"/modelpvaluesstandarized",i,".txt",sep="")
...
edges=list()
for (i in 1:nv){
  formula=paste(getwd(),"/modelrandomstandarized",i,".txt",sep="")
