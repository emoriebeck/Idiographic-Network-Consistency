...
setwd("~/Dropbox/network/PAIRS/Wave 4/Matlab/raw")
getwd()
...
#%###########################################################%#
formula_data<-paste(getwd(),"/esm_w4_networks.csv",sep="")
...
pvalueslist=list()
for (i in 1:nv){
  formula1=paste(getwd(),"/modelfixedraw",i,".txt",sep="")
  fixed <-as.vector(unlist(read.table(formula1, quote="\"")))
  
  formula2=paste(getwd(),"/modelpvaluesraw",i,".txt",sep="")
...
edges=list()
for (i in 1:nv){
  formula=paste(getwd(),"/modelrandomraw",i,".txt",sep="")
