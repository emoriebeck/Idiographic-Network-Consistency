...
load('esmw7centered.mat')  
indiv=esmw7centered(:,1);
indivindiv=unique(indiv);
obs = size(esmw7centered,1);
esmw7centered2 = [nan(length(indivindiv),size(esmw7centered,2)); esmw7centered];
esmw7centered = [esmw7centered; nan(length(indivindiv),size(esmw7centered,2))];
esmw7centered(obs+1:end,1) = indivindiv;
esmw7centered2(1:length(indivindiv),1) = indivindiv;
[~, order] = sort(esmw7centered(:,1));
esmw7centered = esmw7centered(order,:);
[~, order] = sort(esmw7centered2(:,1));
esmw7centered2 = esmw7centered2(order,:);
esmw7centered = [esmw7centered, esmw7centered2(:,2:16)];

idx=(esmw7centered==9999); %find nans and replcae them with NAN 
esmw7centered(idx)=NaN;



indiv=esmw7centered(:,1);
dataY=esmw7centered(:,2:16);
dataX=esmw7centered(:,17:31);
