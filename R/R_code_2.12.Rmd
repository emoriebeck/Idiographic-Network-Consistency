...
##Load Data
```{r}
wave1_all <- read.csv(sprintf("%s/data/esm_w1_redacted.csv",     data_path)) %>% tbl_df
wave4_all <- read.csv(sprintf("%s/data/esm_w4_redacted.csv", data_path)) %>% tbl_df
wave7_all <- read.csv(sprintf("%s/data/esm_w7_redacted.csv", data_path)) %>% tbl_df
```
...
centrality %>% 
    filter(SID %in% c("91339", "22652") &  grepl("trength", measure)) %>%
...
target.ratings.initial.w1 <- tbl_df(read.csv(url(sprintf("%s/data/target_w1_redacted.csv", data_path)))) %>%
  mutate(wave = "1", ts.IDnum.w1 = ifelse(ts.IDnum.w1 < 10000, ts.IDnum.w1 + 10000, ts.IDnum.w1))
target.ratings.initial.w2 <- tbl_df(read.csv(url(sprintf("%s/data/target_w4_redacted.csv", data_path)))) %>%
