...
Left off formatting at the beginning of the consistency section
maybe make some sort of reference for the footnotes
...
data_path <- "~/Box Sync/network/PAIRS/consistency"
esm_codebook <- tbl_df(readxl::read_xlsx(sprintf("%s/data/Codebook.xlsx", data_path), sheet = "ESM"))
```
...
save(gVAR_fit, file = sprintf("%s/R/graphicalVAR_allSubs.RData", data_path))
```
...
load(sprintf("%s/R/graphicalVAR_allSubs.RData", data_path))
...
gVAR_data <- gVAR_fit %>% 
  ...
  select(-gVAR_fit)
save(gVAR_data, file = "~/Box Sync/network/PAIRS/consistency/R/app_data.RData")
...
ggsave(sprintf("%s/results/plots/centrality_plot_1+2.png", data_path), width = 5, height = 5)
```
...
contemp_cors_long %>%
  ...
  theme(strip.text = element_text(face = "bold"),
        ...
        legend.position = "bottom")
ggsave(file = sprintf("%s/results/plots/Rank_Edge_Cors_heatmap.png", data_path), width = 9, height = 6)
```
...
save(gVAR_fit_split, file = "~/Box Sync/network/PAIRS/consistency/results/split_half_gVAR_fit.RData")
```
...
##split half networks
load("~/Box Sync/network/PAIRS/consistency/results/split_half_gVAR_fit.RData")
...
plot_fun <- function(df, type){
  ...
  ggsave(sprintf("%s/results/plots/%s_split_reliability.png", data_path, type), 
         width = 6, height = 4)
  print(plot)
  return(plot)
}
...
```{r}
load(sprintf("%s/R/PAIRS_networks_pers_FINAL.RData", data_path))
...
ggplot(ip_cors, aes(x = cors)) + 
  ...
  theme(axis.text = element_text(face = "bold", size = rel(1.2)),
        ...
        strip.text = element_text(face = "bold", size = rel(1.2)),
        plot.title = element_text(face = "bold", size = rel(1.2), hjust = .5))
ggsave(sprintf("%s/results/plots/ipsative_cor.png", data_path), 
       width  = 6, height = 3)
```
...
ggsave(sprintf("%s/results/plots/centrality_plot.png", data_path), width = 5, height = 3.5)
```
...
target.ratings.initial.w1 <- tbl_df(read.csv("~/Box Sync/network/PAIRS/consistency/data/Wave 1/target_w1_RENAMED.csv")) %>%
  mutate(wave = "1", ts.IDnum.w1 = ifelse(ts.IDnum.w1 < 10000, ts.IDnum.w1 + 10000, ts.IDnum.w1))
target.ratings.initial.w2 <- tbl_df(read.csv("~/Box Sync/network/PAIRS/consistency/data/Wave 4/home_w4_RENAMED.csv")) %>%
