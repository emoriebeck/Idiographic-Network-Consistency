...
data_path <- "https://github.com/emoriebeck/PAIRS-Network-Stability"
...
load(sprintf("%s/results/graphicalVAR_allSubs.RData", data_path))
...
gVAR_data <- gVAR_fit %>% 
  ...
  select(-gVAR_fit)
save(gVAR_data, file = sprintf("%s/R/app_data.RData", data_path))
...
##split half networks
load(sprintf("%s/results/split_half_gVAR_fit.RData", data_path))
...
```{r}
load(sprintf("%s/results/PAIRS_networks_pers_FINAL.RData", data_path))
...
ip_temp_cent_cors %>%
  ...
  theme(plot.title = element_text(hjust = .5),
        ...
        axis.title = element_text(face = "bold", size = rel(1.2)),
        strip.text = element_text(face = "bold", size = rel(1.2)))
ggsave(sprintf("%s/results/plots/centrality_ipsative_cor.png", data_path), 
       width  = 6, height = 3)
```
